...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 px-4 md:px-8 py-4 pointer-events-none">
      <motion.nav
        className={`mx-auto max-w-7xl pointer-events-auto flex items-center justify-between px-6 py-3 rounded-full transition-all duration-500 ${isScrolled ? "bg-white/70 backdrop-blur-xl shadow-lg border border-white/50" : "bg-transparent"
          }`}
      ...
      <div
        className={`fixed inset-0 bg-slate-900/10 backdrop-blur-md z-[60] transition-opacity duration-300 md:hidden ${isOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
          }`}
        ...
        <nav className="flex flex-col gap-6">
          {links.map((link) => (
            <Link
              ...
              href={link.href}
              onClick={() => setIsOpen(false)}
              className={`text-lg font-bold transition-colors ${pathname === link.href ? "text-blue-600" : "text-slate-600"
                }`}
            >
              {link.name}
            </Link>
